...
class common:
    ...
    def parse_prediction_results(raw_prediction_results, unhash_dict, topk: int = 5) -> List['MethodPredictionResults']:
        prediction_results = []
        # print("\n\n\n\nraw_prediction_results")     #Apna Kaam
        # print(raw_prediction_results)               #Apna Kaam
        # print("\n\nRESULTS")
        # print(raw_prediction_results[2][1])            #Apna Kaam
        # for single_method_prediction in raw_prediction_results:
        #     print("\n\n\n Single method Prediction") #Apna Kaam
        #     print(single_method_prediction)          #Apna Kaam
        ...
        current_method_prediction_results = MethodPredictionResults(raw_prediction_results[0])
        for i, predicted in enumerate(raw_prediction_results[1]):
            # if predicted == special_words.OOV:
            #     continue
            suggestion_subtokens = common.get_subtokens(predicted)
            current_method_prediction_results.append_prediction(
                suggestion_subtokens, raw_prediction_results[2][i])
        topk_attention_per_context = [
            (key, raw_prediction_results[3][key])
            for key in sorted(raw_prediction_results[3],
                                key=raw_prediction_results[3].get, reverse=True)
        ][:topk]
        for context, attention in topk_attention_per_context:
            token1, hashed_path, token2 = context
            if hashed_path in unhash_dict:
