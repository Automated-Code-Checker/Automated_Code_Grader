...
from model_implementation import code2vec_model
import sys
import os
import torch
#import ptvsd`~
...
if __name__ == '__main__':
    ...
    parser.add_argument("-w2v", "--save_word2v", dest="save_w2v",
                        help="path to save file", metavar="FILE", required=False)
    # parser.add_argument("-context_vector_size", "--context_vector_size", dest="context_vector_size",
    #                     help="context vector", metavar="FILE", required=False)
    ...
    # parser.add_argument('--MAX_CONTEXTS', action='store_true', default=200, required=False)
    args = parser.parse_args()

    print(args, " pppppp args")

    config = Config.set_defaults(args)


    # model = code2vec_model(config)
