...
class code2vec_model(nn.Module):
  ...
  def __init__(self, 
    ...
    self.linear = nn.LSTM(self.path_embedding_dim + 2 * self.val_embedding_dim, self.embedding_dim, bias = False)
    ...
    ## 4. Prediction
    self.output_linear = nn.LSTM(self.embedding_dim, self.labels_num, bias = False)
  ...
  def forward(self, starts, paths, ends):
    ...
    if self.bert:
      ...
      _, code_vector = self.bert(attention_mask=mask, inputs_embeds=context_vec ,return_dict=False)  #Apna Changes
    else:
